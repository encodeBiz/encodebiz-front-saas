...
export default function OnboardingCard({ children, serviceData, title, description, description2, image, onPress, heightCard = 250, right = -10, top = -60, width = 705, height = 422 }: OnboardingCardProps) {
  const { user } = useAuth()
    ...
    <Paper sx={{ ...styles.base, minHeight: heightCard }}>
      <Box sx={image ? styles.root : styles.rootSimple}>
        <Box sx={styles.container}>
          ...
          {description2 && <Typography sx={styles.subtitle} variant="subtitle1" align="center" >
            {description2}
          </Typography>}
