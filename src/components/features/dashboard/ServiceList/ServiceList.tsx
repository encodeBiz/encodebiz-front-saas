...
const ServiceList = () => {
    ...
    if (entityServiceList.filter(e => !(e.id === 'passinbiz' && process.env.NEXT_PUBLIC_ENV === 'prod')).filter(e => entitySuscription.filter((es) => es.serviceId === e.id).length === 0).length > 0)
        return (
