...
const ColumnCard = ({ icon, title, text1, text2, items }: { icon: ReactNode, title: string, text1: string, text2: string, items: Array<string> }) => {
    const t = useTranslations()
    return <Box width={'100%'} display={'flex'} flexDirection={"column"} gap={2} sx={{ background: (theme) => theme.palette.primary.light }} >
        <Box borderRadius={2} p={2} display={'flex'} flexDirection={"column"} gap={1} sx={{ background: (theme) => theme.palette.primary.dark, color: "#FFF" }}>
            <Box borderRadius={2} p={2} display={'flex'} flexDirection={"column"} alignItems={'flex-start'} gap={2}  >
                <Box display={'flex'} flexDirection={'row'} alignItems={'center'} justifyContent={'center'} gap={2}>
                    {icon}
                    <Typography variant="body1" fontWeight={400} fontSize={32}>{title}</Typography>
                </Box>
                <Box>
        ...
        <Box px={4} py={2} display={'flex'} flexDirection={"column"} gap={1}   >
            <Typography color="#001551" variant="body1" fontSize={22} fontFamily={karla.style.fontFamily}>
                {t('cards.card3.allow')}
            </Typography>
            <ul>
                {items.map((e, i) => <li key={i} ><Typography variant="body1" fontSize={16} fontFamily={karla.style.fontFamily}>
                    {e}
                </Typography></li>)}
            </ul>
        </Box>
    ...
    return <Box display={'flex'} flexDirection={"column"} justifyContent={'space-between'} p={2}  >
        <Box width={'100%'} display={'flex'} flexDirection={"column"} gap={1} justifyContent={'space-between'} alignItems={'flex-start'} >
            ...
            <Box display={'flex'} flexDirection={"row"} gap={5}  >
                <Box borderRadius={2} width={'100%'} display={'flex'} flexDirection={"row"} gap={2} >

                    <ColumnCard
                        icon={<svg width="66" height="70" viewBox="0 0 66 70" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        title={'PassBiz  (Muy Pronto)'}
                        text1={t('cards.card3.text4')}
                        text2={t('cards.card3.text5')}
                        items={[
                            t('cards.card3.text8'),
                            t('cards.card3.text9'),
                    ...
                    <ColumnCard
                        icon={<svg width="61" height="75" viewBox="0 0 61 75" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        title={'CheckBiz'}
                        text1={t('cards.card3.text12')}
                        text2={t('cards.card3.text13')}
                        items={[
