  ...
  return (
    <Container maxWidth="lg">
      ...
      {open.type === CommonModalType.LOGS && !!logDetail && (
        <Dialog open onClose={() => closeModal(CommonModalType.LOGS)} maxWidth="sm" fullWidth>
          <DialogTitle sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            {t("core.label.viewDetails")}
            <IconButton size="small" onClick={() => closeModal(CommonModalType.LOGS)}>
          ...
          <DialogContent dividers>
            <Stack spacing={2.5}>
              ...
              <Box>
                ...
                <Button size="small" variant="outlined" onClick={() => onGoMap(logDetail.geo?.lat as number, logDetail.geo?.lng as number)} sx={{ mt: 1 }}>
                  {t('sucursal.map')}
                </Button>
              </Box>
          ...
          <DialogActions>
            <Button onClick={() => closeModal(CommonModalType.LOGS)}>{t('core.button.close') ?? 'Cerrar'}</Button>
          </DialogActions>
        </Dialog>
      )}
